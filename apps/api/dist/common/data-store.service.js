...
let DataStoreService = class DataStoreService {
    ...
    updateUserStatus(id, status, statusMessage) {
    ...
    createConversation(participants, workplaceId = 'wp-teamchat-main') {
        const existing = this.conversations.find((c) => c.participants.length === participants.length &&
    ...
    deleteMessage(id) {
        ...
        this.messages = this.messages.filter((m) => m.id !== id && m.parentMessageId !== id);
    ...
    toggleReaction(messageId, emoji, userId, userName) {
    ...
    search(query) {
        ...
        const matchedMessages = this.messages.filter((m) => m.content.toLowerCase().includes(q));
        const matchedChannels = this.channels.filter((c) => c.name.toLowerCase().includes(q) ||
            (c.description && c.description.toLowerCase().includes(q)));
        const matchedUsers = this.users.filter((u) => u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q));
        return {
    ...
    addChannelMembers(channelId, userIds) {
